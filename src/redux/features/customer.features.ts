    ...
    endpoints: (builder) => ({
        ...
        getSingleCustomerInvoice: builder.query({
            query: (payload: {
                ...
                url: `/customer/invoices/${payload.customerId}` +
                    `?date=${payload.startDate}_${payload.endDate}` +
        ...
        getSingleCustomerDelivery: builder.query({
            query: (payload: {
                ...
                url: `/customer/deliveries/${payload?.customerId}` +
                    `?date=${payload.startDate}_${payload.endDate}` +
                    `&page=${payload.query?.page}` +
                    `&limit=${payload.query?.limit}`,
            }),
        ...
        getSingleCustomerDueCollection: builder.query({
            query: (payload: {
                ...
                url: `/customer/dues/${payload?.customerId}` +
                    `?date=${payload.startDate}_${payload.endDate}` +
                    `&page=${payload.query?.page}` +
                    `&limit=${payload.query?.limit}`,
            }),
